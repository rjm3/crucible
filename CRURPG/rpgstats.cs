...
function Game::refreshClientScore(%clientId)
{
	...
	if($displayPingAndPL) {
		...
		if (%lvl < 230)
			Client::setScore(%clientId, "%n\t" @ %z @ "\t " @ %lvl @ "\t" @ getFinalCLASS(%clientId), (%lvl + %alvl));
		else
			Client::setScore(%clientId, "%n\t" @ %z @ "\t +" @ %alvl @ "\t" @ getFinalCLASS(%clientId), (%lvl + %alvl));
	}
}
...
function DoRemort(%clientId)
{
	...
	SideBonus(%clientId,$LckBonus,$initLCK,0,0);
	storeData(%clientId, "SPcredits", $initSPcredits, "inc");
	SideBonus(%clientId,$SpBonus,$initSPcredits,0,0);
...
function ViewStats(%clientId,%stats)
{
	...
	%msg = "";
	if (%stats == 1) {
		%msg = %msg @ "<f1><jc>" @ Client::getName(%clientId) @ ", LEVEL " @ fetchData(%clientId, "LVL") @ "+" @ fetchData(%clientId,"ALVL") @ " " @ fetchData(%clientId, "CruRACE") @ " " @ getFINALClass(%clientId) @ "<f0>\n\n";
		...
		%msg = %msg @ "<f1>Gold Find: <f0>" @ GetBonus(%clientId,$BPGOLDFIND) @ "%\n";
